...
`;let g={xs:"0.75rem",sm:"0.875rem",base:"1rem",lg:"1.125rem",xl:"1.25rem","2xl":"1.5rem","3xl":"1.875rem","4xl":"2.25rem","5xl":"3rem","6xl":"3.75rem","7xl":"4.5rem","8xl":"6rem","9xl":"8rem"};for(let[l,a]of Object.entries(g))e+=`  --tw-text-${l}: ${a};
...
`;if(e.state==="dark"&&(u=`:where(.dark) ${u}`),e.breakpoint){let g=c.theme.screens[e.breakpoint];if(g)return`@media (min-width: ${g}) { ${u} }
`}return u}function k(){let s={layout:new Set,space:new Set,visual:new Set};return document.querySelectorAll("[layout], [space], [visual]").forEach(e=>{["layout","space","visual"].forEach(r=>{let o=e.getAttribute(r);o&&o.split(/\s+/).forEach(u=>{u&&s[r].add(u)})})}),s}function A(s,n){let e=x(n);n.preflight!==!1&&(e+=$());let r=[];for(let[a,m]of Object.entries(s))for(let h of m)r.push({raw:h,attrType:a});let o=[],u=[],g={},{screens:t}=n.theme;for(let a of Object.keys(t))g[a]=[];for(let a of r){let m=M(a.raw,a.attrType);m.state==="dark"?u.push(m):m.breakpoint?(g[m.breakpoint]||(g[m.breakpoint]=[]),g[m.breakpoint].push(m)):o.push(m)}let i=["flex","grid","inline-flex","inline-grid","block","inline","hidden","contents"],l=new Map;for(let a of o)a.attrType&&i.includes(a.property)&&(l.has(a.attrType)||l.set(a.attrType,new Set),l.get(a.attrType).add(a.raw));for(let a of o)e+=v(a.raw,a.attrType);for(let[a,m]of Object.entries(g))if(m.length>0){e+=`
@media (min-width: ${t[a]}) {
...
`,h.add(b.raw)}}for(let b of m)e+="  "+v(b.raw,b.attrType);e+=`}
`}if(u.length>0){let a=n.darkMode||"media";if(a==="media"){e+=`
@media (prefers-color-scheme: dark) {
`;for(let m of u)e+="  "+v(m.raw,m.attrType);e+=`}
`}else{let m=Array.isArray(a)?a[1]:".dark";e+=`
/* Dark Mode */
`;for(let h of u){let b=v(h.raw,h.attrType);e+=b.replace(/^(\[[^\]]+\])/,`${m} $1`)}}}return e}function V(s){let n=document.getElementById("senangstart-jit");n||(n=document.createElement("style"),n.id="senangstart-jit",document.head.appendChild(n)),n.textContent=s}function D(){let s=d(),n=f(s),e=k(),r=A(e,n);V(r),new MutationObserver(()=>{let u=k(),g=A(u,n);V(g)}).observe(document.body,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["layout","space","visual"]}),console.log("%c[SenangStart CSS]%c JIT runtime initialized \u2713","color: #2563EB; font-weight: bold;","color: #10B981;")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",D):D()})();})();
